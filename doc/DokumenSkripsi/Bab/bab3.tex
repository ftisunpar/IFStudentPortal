...
\begin{itemize}
	\item PHPSESSID: diambil dari \textit{cookie} yang di-\textit{set} saat pengalihan ke halaman utama
\end{itemize}
\item Halaman ``Kuliah, UTS dan UAS'' menampilkan jadwal kuliah, UTS, dan UAS semester terkini. Jika ingin melihat jadwal semester sebelumnya, mahasiswa dapat mengklik \textit{combo box} ``select\#tahun\_akd\_sec'' (Gambar \ref{fig:3_case_jadwal_terkini}) kemudian memilih ``option'' yang diinginkan (Gambar \ref{fig:3_case_nilai_pilih}).
		\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{Gambar/case-jadwal-terkini}
			\caption{Jadwal Semester Terkini} 
			\label{fig:3_case_jadwal_terkini}
		\end{figure}
...
\begin{enumerate}
	\item Mengklik \texttt{a} dengan teks ``Seluruh Fakultas'' pada \textit{list} \texttt{ul.hidden}
	\item Setelah diklik, mahasiswa akan diarahkan ke \url{https://studentportal.unpar.ac.id/includes/jadwal.all.php} (Gambar \ref{fig:3_case_jadwal_menu})
	\begin{figure}[H]
			\centering
			\includegraphics[scale=0.5]{Gambar/case-jadwal-menu}
			\caption{Elemen ``a'' dengan teks ``Kuliah, UTS dan UAS'' pada Menu Jadwal} 
			\label{fig:3_case_jadwal_menu}
		\end{figure}
		\item Halaman tersebut menampilkan jadwal seluruh fakultas semester terkini
\end{enumerate}
...
\begin{enumerate}
	...
	\item Setelah diklik, mahasiswa akan diarahkan ke \url{https://studentportal.unpar.ac.id/home/index.logout.php} dengan \textit{cookie} sebagai berikut:
		\begin{itemize}
			\item PHPSESSID: diambil dari \textit{cookie} yang di-\textit{set} saat pengalihan ke halaman utama
		\end{itemize} 
	\item Kemudian dilakukan pengalihan ke \url{https://cas.unpar.ac.id/logout?service=https\%3A\%2F\%2Fstudentportal.unpar.ac.id} (Gambar \ref{fig:3_case_logout_expire}) lalu mengadaluarsakan \textit{cookie} CASTGC dan CASPRIVACY yang sebelumnya di-\textit{set} saat pengalihan ke halaman utama
			\begin{figure}[H]
				\centering
				\includegraphics[scale=0.5]{Gambar/case-logout-expire}
				\caption{} 
				\label{fig:3_case_logout_expire}
			\end{figure}
	kembali ke \url{https://studentportal.unpar.ac.id/} (Gambar \ref{fig:3_case_logout})
		\begin{figure}[H]
				\centering
